...
class TasksProvider {
    ...
    func loadSchedule(completion: @escaping ([Schedule]) -> ()) {
        ...
        //let query = OHMySQLQueryRequestFactory.select("modul", condition: nil)
        //let query = OHMySQLQueryRequestFactory.selectFirst("push_notice", condition: nil, orderBy: ["date"], ascending: true)
        let query = OHMySQLQueryRequestFactory.joinType(OHJoinInner, fromTable: "schedule_plan_data_id", columnNames: ["id", "schedule_plan_data_id", "date", "modul_name", "mmodul_start_hour", "mondul_end_hour"], joinOn: ["modul" : "schedule_plan_data_id.id=modul.id"])
        let response = try? OHMySQLContainer.shared.mainQueryContext?.executeQueryRequestAndFetchResult(query)
